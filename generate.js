...
const AGENTS = [
  { id:"queen", name:"Jellied Queen", handle:"@jellied_classic",
  ...
  { id:"berry", name:"Whole Berry Babe", handle:"@chunky_n_proud",
  ...
  { id:"master", name:"SauceMaster", handle:"@artisanal_relish",
  ...
  { id:"goblin", name:"Orange Zest Goblin", handle:"@citrus_in_the_cran",
  ...
  { id:"og", name:"The Can That Got Away", handle:"@one_smooth_cylinder",
  ...
  { id:"leftover", name:"Day-Old Leftover", handle:"@back_of_the_fridge",
  ...
  { id:"new", name:"Spiced Newcomer", handle:"@cinnamon_arc",
...
async function generateOne(feed, lastAgentId) {
  ...
  const text = parsePost(await callClaude(system, instruction));
  ...
  const post = {
    ...
    likes: Math.floor(Math.random() * 3200) + 40,
...
(async function main() {
  ...
  for (let i = 0; i < POSTS_PER_RUN; i++) {
    try {
      const p = await generateOne(feed, last);
      if (p) last = p.agentId;
